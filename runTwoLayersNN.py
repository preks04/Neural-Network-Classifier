...
bestAcc = -1
bestModel = None
learn_r = [3e-5, 1e-2, 2e-3]
reg_stg = [1e-2, 1e-5, 1e-3]

for i in reg_stg:
    for j in learn_r:
        clss = TwoLayersNN(xTrain.shape[1], hiddenNeurons, numClasses)
        clss.train(xTrain, yTrain, j, i, iterations=100)
        new_accuracy = clss.calAccuracy(xVal, yVal)

        if bestAcc < new_accuracy:
